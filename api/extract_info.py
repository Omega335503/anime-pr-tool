...
## categoryの分類ルール（必ず以下のいずれかを設定すること。正しく分類することが最重要）
- "MTG" … 社内ミーティング・打ち合わせの議事録・決定事項のみ。「ミーティング」「MTG」「打ち合わせ」と明記されている場合のみ
- "プレスリリース" … PR TIMES等でのプレスリリース配信
- "X更新" … X(Twitter)での投稿・更新・告知。「X更新」「ツイート」「X投稿」「RT」「リポスト」等はこれ
- "SNS更新" … YouTube・Instagram・TikTok等のX以外のSNSでの投稿・更新。「YouTube更新」「Instagram投稿」「TikTok」等はこれ
- "制作物" … PV・ティザー・CM・キービジュアル等の映像や画像の制作物の公開
- "WEB/HP" … 公式サイト更新・HP公開
- "イベント" … リアルイベント・先行上映会・ステージ
- "アニメ制作" … アフレコ・納品・制作マイルストーン等のアニメ制作工程
- "メディア" … メディア取材・インタビュー・雑誌掲載
- "コラボ/グッズ" … コラボ・グッズ・タイアップ
- "その他" … 上記に該当しないもの

## 分類の注意点
- 「X更新」「ツイート」「X投稿」「リポスト」「RT」「フォロー&RT」「プレキャン」→ 必ず "X更新"
- 「YouTube更新」「Instagram投稿」「TikTok」→ 必ず "SNS更新"
- 「ミーティング」「MTG」→ 必ず "MTG"
- 「PV公開」「KV公開」→ "制作物"
- 「プレスリリース」「PR」→ "プレスリリース"
- 迷ったら内容で判断し、SNSプラットフォームでの更新は全て "SNS更新"

## titleの付け方（最重要）
titleは施策の「大分類」のみを簡潔に書く。具体的内容はitemsに入れる。
- ✅ 良い例: title="プレスリリース", items=[{label:"キービジュアル②公開"}, {label:"PV②公開"}]
- ✅ 良い例: title="YouTube更新", items=[{label:"VTuber動画② アップロード"}]
- ❌ 悪い例: title="キービジュアル②・PV②等プレスリリース & YouTube/X更新" ← 混ぜすぎ
- ❌ 悪い例: title="VTuber動画② YouTube更新" ← 内容とタイトルが混ざっている

## エントリの分割ルール（最重要）
- 同じ日付でもカテゴリが異なる施策は必ず**別エントリ**に分ける
- 例: 9/4に「プレスリリース」と「YouTube更新」と「X更新」がある場合 → 3つのエントリに分割
  - {date:"9/4", category:"プレスリリース", title:"プレスリリース", items:[...]}
  - {date:"9/4", category:"SNS更新", title:"YouTube更新", items:[...]}
  - {date:"9/4", category:"SNS更新", title:"X更新", items:[...]}
- 絶対に異なるカテゴリの施策を1つのエントリにまとめないこと

## 日付のルール
- 日付は必ず「YYYY/MM/DD」形式（例: "2026/09/04"）で出力すること
- 年が資料に明記されていない場合は文脈から推測して補完する（例: アニメが2026年10月放送なら、それ以前の準備は2025年〜2026年）
- 日付が不明な項目は date を "日付未定" とする
- 月/日のみ書かれている場合でも必ず年を補完すること

## その他のルール
- 資料に書かれている情報をそのまま使うこと（勝手に言い換えない）
- 日付がある項目は日付順に並べる
- items内のlabelは具体的な施策内容、childrenは補足情報（インデントされた内容）
- childrenがなければ空配列 []
- 日付が不明な項目は date を "日付未定" とする
